...
//			processQuery("test-d0003", "hir_sample_customer", "mysql", query[6]);
		...
		// processQuery("devinstance3", "jxntm4_2_54", "mysql", query[0]);
		...
		if (!rs.next()) {
			...
			for (int i = 1; i <= colCounter; i++) {
				while (rs.next()) {
					...
					// Setting cell height 
					if ( (rs.getString(7).split(System.getProperty("line.separator")).length > colHeight)) {
						colHeight = rs.getString(i).split(System.getProperty("line.separator")).length;
					}
